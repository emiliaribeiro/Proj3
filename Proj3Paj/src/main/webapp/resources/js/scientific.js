...
/**
 * Se o botão numérico for pressionado vai para o display
 *  digito: digito numerico a acrescentar [0-9]
 */
...
/**
 * Se o botão "." ou um operador pressionado vai para o display
 * ponto ou operador a acrescentar [.+-/*]
 */
...
//				//testar que nao existe outro ponto nos caracteres anteriores até aparecer um operador 
		...
		//se digito anterior for um abrir parentices
			...
			if (simbol=="-") {
				...
				alert ('Erro: não pode escrever o operador '+simbol+' a seguir à abertura de parêntesis!');
			}
		}
...
/**
 * se o caracter for [0-9] retorna true. 
...
/**
 * se o caracter for [-+*\/] retorna true. 
 * caso contrário false.
 */
...
/**
 * se o caracter é uma funação retorna True caso contrário retorna false
 */
function isfn(char){
	//alert("TEST: Char lenght: " + char.length);
	if (char.length!=1) {
...
/**
 * Button clear: limpa o último número e operador
 */
function btnclear() {
	var displaytxt = document.getElementById("scientificform:scientificdisplay").value;
...
/**
 * Button clear limpa a expressao 0.0
 */
...
function btnpi() {
	...
	if ( firstdigit == "true" ){
		...
		if (isoperator(lastchar) || isfn(lastchar) || lastchar=="(") {
			...
			alert ('Erro: tem que colocar um operador ou abrir parêntesis antes de PI.');
		}
	}
